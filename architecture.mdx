...
- **Connector setup:** When you connect a database, Ardent inspects and validates it, and gives you connector-level settings: the default database name for new branches, and [any SQL to run on each new branch](/cli/settings).
...
Ardent manages everything from our control plane. The data plane is the only thing that moves.
...
**Managed, your cloud.** Same managed experience, but the data plane deploys into your own cloud account. Your data never leaves your infrastructure. The control plane still manages everything via API; all replication and branch compute runs inside your network.
