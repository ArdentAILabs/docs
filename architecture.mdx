...
- **Ardent Replicator:** Owns the stream from your production database into the read replica: logical replication through a Kafka-backed pipeline. DDL-aware handling for in-flight schema changes so the stream does not stall or silently diverge; ordered application of changes so retries and replays do not produce gaps or duplicates; WAL and slot pressure managed so replication does not destabilize your primary; quarantine, resume, and recovery when something breaks. This is where most home-grown CDC stacks fall apart.
