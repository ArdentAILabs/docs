...
The loop is simple: run preflight, apply the SQL it generates, run preflight again until it passes.
...
<Accordion defaultOpen={false} title="Using Claude Code or Cursor? Paste this prompt and it does the checks for you">
```
...
When preflight passes, continue with the [Quickstart](/quickstart): create the connector and your first branch. One heads-up for that step: tables without a primary key or valid unique index need a [replica identity decision](/cli/connectors#replica-identity-decisions) — the CLI walks you through it, and Ardent never runs DDL on your source automatically.
