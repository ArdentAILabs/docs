...
Note the project name and connector name. If you only have one of each, the CLI auto-selects after login and you can skip the "Select project and connector" step in the workflow below.

</Step>

<Step title="Create an API token">

Create a token in the [Ardent dashboard](https://app.tryardent.com) or with:

```bash
curl -X POST https://api.tryardent.com/v1/orgs/{org_id}/api-keys
```
...
- **`ardent project switch` / `ardent connector switch`** selects which database to branch from. Replace `my-project` and `my-connector` with your names. Skip this step if you only have one project and one connector.
