  ...
  test:
    ...
    steps:
      ...
      - name: Install Ardent CLI
        run: npm install -g ardent-cli@0.0.46
      ...
      - name: Create database branch
        ...
        run: |
          ...
          case "$DATABASE_URL" in
            postgres://*|postgresql://*) ;;
            *) echo "Ardent did not return a Postgres database URL" >&2; exit 1 ;;
          esac
...
- `ARDENT_TOKEN` is passed through `env` to Ardent CLI steps so it is not interpolated directly into shell commands or exposed to the test step.
...
- The workflow validates the captured URL starts with `postgres://` or `postgresql://`, then masks it before writing it to `GITHUB_ENV`.
...
- `timeout-minutes` bounds external-service hangs, and cleanup runs with `if: always()` after branch creation so failed tests still delete the branch.
...
```bash
npm install -g ardent-cli@0.0.46
ardent project switch my-project
ardent connector switch my-connector
