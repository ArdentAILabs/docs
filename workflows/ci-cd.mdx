---
title: "CI / CD"
description: "A fresh database for every PR. Isolated, production-like, torn down after merge."
---

Every PR gets its own fresh database cloned from production, torn down after merge. No shared staging that drifts out of sync. No test runs that corrupt each other.

## Prerequisites

You need an Ardent account with at least one connector set up. If you haven't done that yet, follow the [Quickstart](/quickstart) first.

## Setup

<Steps>
<Step title="Get your project and connector names">

Run these locally to find the values you'll need for the workflow:

```bash
ardent project list
ardent connector list
```

Note the project name and connector name. You will use these in the workflow to explicitly select which database to branch from.

</Step>

<Step title="Create an API token">

Go to [Ardent dashboard > API keys](https://app.tryardent.com/app/settings/api-keys) and create a token.

</Step>

<Step title="Add the token to GitHub">

In your repo, go to **Settings > Secrets and variables > Actions** and click **New repository secret**. Name it **`ARDENT_TOKEN`** and paste your token.

</Step>

<Step title="Add the workflow file">

Copy the YAML below to `.github/workflows/ardent-branch.yml` in your repo, replacing `my-project` and `my-connector` with your values from step 1.

</Step>
</Steps>

## GitHub Actions workflow

```yaml
name: Test with Ardent branch

on:
  pull_request:
    types: [opened, synchronize, reopened]

jobs:
  test:
    runs-on: ubuntu-latest
    env:
      ARDENT_BRANCH: pr-${{ github.event.pull_request.number }}-${{ github.run_id }}
    steps:
      - uses: actions/checkout@v4

      - uses: actions/setup-node@v4
        with:
          node-version: "22"

      - name: Install dependencies
        run: npm ci

      - name: Install Ardent CLI
        run: npm install -g ardent-cli

      - name: Log in to Ardent
        env:
          ARDENT_TOKEN: ${{ secrets.ARDENT_TOKEN }}
        run: ardent login --token "$ARDENT_TOKEN"

      - name: Select project and connector
        run: |
          ardent project switch my-project
          ardent connector switch my-connector

      - name: Create database branch
        run: |
          ardent branch create "$ARDENT_BRANCH"
          echo "DATABASE_URL=$(ardent branch info | awk '/URL:/{print $2}')" >> "$GITHUB_ENV"

      - name: Run tests
        run: npm test

      - name: Delete database branch
        if: always()
        run: ardent branch delete "$ARDENT_BRANCH"
```

**What each step does:**

- **`ardent login --token "$ARDENT_TOKEN"`** authenticates the CLI using your repo secret. Passing the secret through `env` keeps shell metacharacters in the token from being interpreted by the shell.
- **`ardent project switch` / `ardent connector switch`** explicitly selects which project and connector to branch from. A connector represents one Postgres server. Replace `my-project` and `my-connector` with your names. Always set these in CI so the workflow is deterministic.
- **`ARDENT_BRANCH`** defines the branch name for both create and delete steps. The name includes the PR number and run ID so concurrent runs and re-runs don't collide.
- **`ardent branch create`** creates an isolated database branch.
- **`ardent branch info | awk '/URL:/{print $2}'`** extracts the branch connection URL and writes it to `DATABASE_URL` in `GITHUB_ENV`. Your test step reads `DATABASE_URL` like any other environment variable. If your app uses a different env var name, change it in the `echo` line.
- **`npm test`** runs your test suite against the branch. Replace with `pytest`, `pnpm test`, `make test`, or whatever your project uses.
- **`ardent branch delete`** cleans up the branch. `if: always()` ensures this runs even when tests fail.

## Copy this into Claude Code or Cursor

```
Set up Ardent database branching in our GitHub Actions CI.
I have an ARDENT_TOKEN secret already configured.
My project name is [project name] and connector is [connector name].
Create .github/workflows/ardent-branch.yml that creates an Ardent branch
for each PR, sets DATABASE_URL for our tests, runs `npm test`, and deletes
the branch after. Use the docs at https://docs.tryardent.com/workflows/ci-cd
```

## Why this beats a shared staging database

- **Isolation:** each PR can't interfere with other test runs
- **Always in sync:** branched from production, no drift
- **No idle cost:** branches auto-suspend between CI runs
- **Auto cleanup:** `if: always()` means no orphaned environments
