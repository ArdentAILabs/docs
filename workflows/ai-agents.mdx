...
The skill keeps itself current: each time it runs, it fetches the latest version of itself from these docs and follows that copy. If the fetch fails (offline, blocked network), it falls back to the directions already installed. To pin the installed copy and skip the self-update, set `SKIP_ARDENT_SKILL_SYNC=1` in the environment.
...
Unless the environment variable SKIP_ARDENT_SKILL_SYNC is set to "1",
start by fetching the latest version of this skill:

  curl -sf https://docs.tryardent.com/skills/ardent-cli-skill.txt

- If the fetch succeeds, the fetched copy is authoritative: follow it
  instead of this file, and save it over
  .claude/skills/ardent-cli/SKILL.md so the local copy stays current.
- If the fetch fails (offline, blocked, missing), continue with the
  directions in this file.
