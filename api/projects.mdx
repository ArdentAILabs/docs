...
```bash
curl --oauth2-bearer "$ARDENT_TOKEN"   -X POST https://api.tryardent.com/v1/projects   -H 'Content-Type: application/json'   -d '{"name":"my-app","org_id":"org_123"}'
```
...
```bash
curl --oauth2-bearer "$ARDENT_TOKEN"   'https://api.tryardent.com/v1/projects?org_id=org_123'
```

## Update a project

```bash
curl --oauth2-bearer "$ARDENT_TOKEN"   -X PATCH https://api.tryardent.com/v1/projects/project_123   -H 'Content-Type: application/json'   -d '{"name":"my-app-production"}'
```

## Delete a project

```bash
curl --oauth2-bearer "$ARDENT_TOKEN"   -X DELETE https://api.tryardent.com/v1/projects/project_123
```
