...
The API uses bearer tokens. Create them in **Ardent dashboard > Settings > API keys** and store them in a secret manager. Keys start with `sk-ard_live_` (or `sk-ard_test_` in test environments) and go in an `Authorization: Bearer` header on every request.
