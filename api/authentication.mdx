...
The API uses bearer tokens. Create them in **Ardent dashboard > Settings > API keys** and store them in a secret manager. Keys start with `sk-ard_` and go in an `Authorization: Bearer` header on every request.
