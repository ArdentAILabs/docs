...
For Postgres branches, `service_type` is `postgres` — not the connector type `postgresql`.
...
When the branch is cut from a source whose WAL advanced past the snapshot, the response also includes a `warning` string describing the staleness.
