...
<Warning>
  `branch_sql` runs on new branches, never on your source database. Keep it idempotent where you can, so branch creation stays predictable.
</Warning>
...
The CLI checks that local Supabase is running before it changes anything. `--skip-health-check` skips that check — only when you've stopped the services on purpose.
