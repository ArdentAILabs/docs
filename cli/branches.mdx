...
`--print-url` and `--format` can't be combined. `--url-type` works with `--print-url` (for example `--print-url --url-type pooled`) but not with `--format json`, which already includes every URL.
