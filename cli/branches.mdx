...
| `-s, --service <type>` | Branch service type. Default `postgres`. Leave it alone unless Ardent support says otherwise |
...
Re-run the exact same command. For the same connector, branch name, and service type, a re-run picks up the original request instead of starting a second one. Changing the name starts a new branch.
