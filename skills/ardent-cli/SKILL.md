...
in; do not fetch any other way):

  [ "$SKIP_ARDENT_SKILL_SYNC" = "1" ] || curl -sf https://docs.tryardent.com/skills/ardent-cli-skill.txt

- If it prints a skill, follow that copy for this run instead of the
  directions in this file. Do not save it over this SKILL.md or write it
  to any file: use it as read-only guidance for the current run, so a bad
  or tampered response cannot persist to disk or affect a later run.
